...
(function(Object, support, undefined)
{
  ...
  {
    let key;
    const hasOwn = support.hasOwnProperty;
...
(function(window, undefined)
{
  ...
  /**
	 ...
	 */
  const include = function(namespaceString, required)
  {
...
(function(window, Object)
{
  ...
  /**
	 ...
	 */
  const mixin = function(target, superClass)
  {
    if (true && !superClass)
    {
    ...
    for (const prop in p)
    {
      ...
      const propDesc = Object.getOwnPropertyDescriptor(p, prop);
      if (propDesc)
      {
...
(function(undefined)
{
  ...
  const p = extend(EventDispatcher);
...
(function(global, doc, undefined)
{
  ...
  /**
	 ...
	 */
  const PageVisibility = function(onFocus, onBlur)
  {
    /**
  ...
  const isIE9 = !_visibilityChange && doc.onfocusin !== undefined;
...
(function(undefined)
{
  ...
  /** 
	 ...
	 * @readOnly
	 */
  let WEB_STORAGE_SUPPORT = window.Storage !== undefined;
  ...
  SavedData.reviver = function(key, value)
  {
    ...
    {
      const _class = include(value.__classname, false);
      if (_class)
      {
        const rtn = new _class();
...
{
  // Include class
  const SavedData = include('springroll.SavedData');
  ...
  /**
	 ...
	 */
  const SavedDataHandler = function() {};

  // Reference to prototype
  const p = extend(SavedDataHandler);
...
(function(undefined)
{
  const Debug = include('springroll.Debug', false);
...
{
  //Import classes
  let EventDispatcher = include('springroll.EventDispatcher'),
  ...
  var Container = function(iframeSelector, options)
  {
    ...
    // Setup plugins
    const plugins = Container._plugins;
  ...
  //Reference to the prototype
  const s = EventDispatcher.prototype;
  const p = EventDispatcher.extend(Container);
  ...
  p._internalOpen = function(path, options)
  {
    ...
    {
      const playOptionsQueryString = 'playOptions=' + window.encodeURIComponent(JSON.stringify(options.playOptions));
...
(function()
{
  ...
  /**
	 ...
	 */
  const ContainerPlugin = function(priority)
  {
    if (!Container)
    {
...
(function()
{
  const SavedData = springroll.SavedData;

  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(100);
...
(function()
{
  ...
  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(70);
  ...
  plugin.setup = function()
  {
    ...
    this.setCaptionsStyles = function(styles, value)
    {
      ...
      if (true)
      {
        const colorReg = /^(black|white|red|yellow|pink|blue)(-semi)?$/;
        const backgroundReg = /^none|((black|white|red|yellow|pink|blue)(-semi)?)$/;
        const sizeReg = /^(xs|sm|md|lg|xl)$/;
        const edgeReg = /^(raise|depress|uniform|drop|none)$/;
        const fontReg = /^(georgia|palatino|times|arial|arial-black|comic-sans|impact|lucida|tahoma|trebuchet|verdana|courier|console)$/;
        const alignReg = /^(top|bottom)$/;
    ...
    {
      const styles = this._captionsStyles;
...
(function()
{
  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(90);
...
(function()
{
  const PageVisibility = include('springroll.PageVisibility');
  ...
  /**
	 ...
	 * @private
	 */
  const onKeepFocus = function(event)
  {
...
(function()
{
  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(50);
...
(function()
{
  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(80);
  ...
  plugin.setup = function()
  {
    ...
    for (let i = 0; i < this.pauseButton.length; i++)
    {
    ...
    Object.defineProperty(this, 'paused',
      {
        set: function(paused)
        {
          if (!this._disablePause)
          {
            ...
            for (let i = 0; i < this.pauseButton.length; i++)
            {
              const element = this.pauseButton[i];
...
(function()
{
  const Features = include('springroll.Features');

  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(30);
  ...
  plugin.setup = function()
  {
    ...
    this.openRemote = function(api, options, playOptions)
    {
      ...
      {
        const err = Features.test(release.capabilities);
...
(function()
{
  ...
  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(60);
...
(function()
{
  const SavedDataHandler = include('springroll.SavedDataHandler');

  /**
	 * @class Container
	 */
  const plugin = new springroll.ContainerPlugin(40);
